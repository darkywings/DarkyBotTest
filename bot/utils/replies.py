...
class Replies:
    ...
    RP_DENIED_FOR_USERS = ("❗️ Этот пользователь запретил использование РП от других пользователей по отношению к себе")

    RP_DENIED_FOR_BOTS = ("❗️ Этот пользователь запретил использование РП от ботов по отношению к себе")

    RP_DENIED_FOR_ALL = ("❗️ Этот пользователь запретил использование РП по отношению к себе")

    USER_NOT_FOUND = ("⚠️ Я не смогла найти данного пользователя в этом чате, вероятно он ни разу не состоял в этой беседе или не активил")

    USER_IS_NOT_IN_CHAT = ("⚠️ Пользователь не находится в чате для выполнения этой команды")

    NICKNAME_TOO_LONG = ("⚠️ Никнейм не может быть длиной более 25 символов")

    NICKNAME_TAKEN = ("❗️ Данный никнейм занят другим пользователем. Попробуйте другой")

    NICKNAME_INVALID_CHARS = ("❗️ В никнейме не могут содержаться символы, такие как: \"$\", \"|\", \"[\", \"]\"")

    CANT_DO_FOR_GROUPS = ("❌ Эту команду нельзя использовать по отношению к сообществам")

    CANT_DO_FOR_BOT = ("❌ Эту команду нельзя использовать по отношению ко мне")

    SELECTED_MEMBER_IS_ADMIN = ("⚠️ Данный пользователь является администратором беседы")

    ALREADY_BANNED = ("⚠️ Данный пользователь уже заблокирован в этой беседе")